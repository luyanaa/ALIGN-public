...
def gen_param(subckt, primitives, pdk_dir):
    ...
    if generator_name == 'CAP':
        ...
        # TR-1um compaction: height is in fin units (1 fin = Fin_pitch). The MOS
        # generator's internal track math (grid_y0=1, grid_y1=mpc-5, gate pad at
        # mpc-3..mpc-1, gate V1 at mpc-2, M2 pin tracks 1..2, body via above the
        # unit cell) is only layout-legal for m2PerUnitCell >= 10: smaller values
        # put the gate pad/V1 between the S/D M1 straps (fabric checker:
        # "via must touch metal"). mpc = height*Fin_pitch/M2_pitch, so with
        # Fin_pitch=50nm / M2_pitch=7000nm one track = 140 fins -> floor 1400.
        m2_pitch_nm = next((l["Pitch"] for l in pdk_data["Abstraction"] if l["Layer"] == "M2"), 7000)
        fin_per_track = m2_pitch_nm // int(design_config["Fin_pitch"])
        height = max(10 * fin_per_track, ((width + 32 + fin_per_track - 1) // fin_per_track) * fin_per_track)
        block_args = {
            'primitive': generator_name,
            'value': width,
            'height': height,
